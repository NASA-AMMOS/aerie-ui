...
def post_payload(run_id, case_id, payload):
    ...
    client.send_get(f'/get_cases/&suite_id={suite_id}')
...
def xml_node_to_description(xml_node):
    if (xml_node_contains_member_once_with_text(xml_node, 'failure')):
        return xml_node.findall('failure')[0].text
    else:
        return 'No message'
    ...
    return [
        {
            ...
            'status': 'PASS' if len(test_node.findall('failure')) == 0 else 'FAIL',
            'time': str(int(float(test_node.get('time'))))
        }
