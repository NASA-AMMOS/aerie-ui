...
export const cyrb53a = function(str: string, seed = 0) {
  ...
  export function getRowIdExternalSourceSlim(externalSourceSlim: ExternalSourceSlim) {
    // https://stackoverflow.com/questions/40958727/javascript-generate-unique-number-based-on-string
    return cyrb53a(externalSourceSlim.pkey.derivation_group_name + externalSourceSlim.pkey.key + externalSourceSlim.pkey.source_type_name)
  }

  export function getRowIdExternalSource(externalSourcePkey: ExternalSourcePkey) {
    // https://stackoverflow.com/questions/40958727/javascript-generate-unique-number-based-on-string
    return cyrb53a(externalSourcePkey.derivation_group_name + externalSourcePkey.key + externalSourcePkey.source_type_name)
  }

  export function getRowIdExternalEvent(externalEventPkey: ExternalEventPkey) {
    // https://stackoverflow.com/questions/40958727/javascript-generate-unique-number-based-on-string
    return cyrb53a(externalEventPkey.derivation_group_name + externalEventPkey.source_key + externalEventPkey.event_type_name + externalEventPkey.key)
  }