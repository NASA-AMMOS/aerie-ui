...
<div class="card st-typography-label">
  <div class="card--row card--title-row">
    <div class="card--title t-typography-medium">
      <LightningCharge/>{newSources.length} new files have been uploaded
    </div>
    <slot name="right">
      <button
  ...
  <div class="card--body st-typography-body">
    {#each Object.keys(mappedSources) as sourceType}
      <Collapse title={sourceType} tooltipContent={sourceType} defaultExpanded={true}>
        {#each Object.keys(mappedSources[sourceType]) as derivationGroup}
          <Collapse title={derivationGroup} tooltipContent={derivationGroup} defaultExpanded={true}>
...
<style>
  .card {
    ...
    border: 1px solid var(--border-color, rgba(249, 191, 29, 0.48));
    border-radius: 4px;
  ...
  .card--title-row {
    background: var(--title-bg-color, rgba(255, 245, 100, 0.24));
    padding: 4px 0px;
  }
